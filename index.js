...
const displayWordDetails = (word) => {
    ...
    <div>
        <h2 class="text-2xl font-bold">${word.word} (<i class="fa-solid fa-microscope"></i>   :${word.pronunciation})</h2>
    </div>
        <div>
...
const displayLevelWord = (words) => {
    ...
    words.forEach(word =>{
      ...
      <h2 class="font-bold text-2xl">${word.word}</h2>
        ...
        <div class="text-2xl font-medium font-bangla">"${word.meaning? word.meaning : "অর্থ পাওয়া যায়নি"} / ${word.pronunciation}"</div>
...
const displayLessons = (lessons) => {
    // 1 step: get the container
   const levelContainer = document.getElementById("level-container");
   levelContainer.innerHTML = "";
    //    2 step: get into every lessons
    for(let lesson of lessons){
         // 3 step: create element
        const div = document.createElement("div");
        div.innerHTML = `
         <button id="lesson-btn-${lesson.level_no}" onclick="loadLevelWord(${lesson.level_no})" class="btn btn-outline btn-primary lesson-btn">
         <i class="fa-solid fa-book-open">
         </i>Leasson - ${lesson.level_no}</button>
        
        `;
    // 4 step: set appendchild
